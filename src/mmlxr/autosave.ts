...
export class AutosaveStatic implements LazySingleton {
	...
	async confirmUnprotected(): Promise<void> {
		let entry: IAutosaveEntry = await this.store();
		if (entry.isProtected || !entry.mml.match(/\S/)) {
			return;
		}
		let protect: boolean = await UI.confirm(
			L('Confirm'),
	...
	async store(rescue: boolean = false): Promise<IAutosaveEntry> {
		this.cancelLazyStore();
		...
		if (!entry.mml.match(/\S/)) return entry;
		
		var lsKey = `${STORAGE_AUTOSAVE_KEY}:${entry.id}`;
		localStorage.setItem(lsKey, JSON.stringify(entry));
		
		let autosave: NdxdbStore = await this.db.tx('autosave');
		console.log('autosaving', rescue ? 'as rescue' : undefined);
		if (entry.mml.match(/\S/) || entry.isProtected)
			await autosave.put(entry);
		else
			await autosave.delete(entry.id);
		localStorage.removeItem(lsKey);
		if (rescue) return;
	...
	async openLoader(ignoreEmpty: boolean = false): Promise<void> {
		App.stop();
		App.resetTimediff(true);
		...
		let data: IAutosaveEntry[];
		await this.confirmUnprotected();
		let entries: IAutosaveEntry[] = await this.collectEntries(true);
		if (entries.length == 0) {
			if (!ignoreEmpty) toastr.warning(L('No autosave stored'));
			return;
		}
		let entry: IAutosaveEntry = await this.createSelector(entries);
		console.log(entry);
		if (!entry) return;
	...
	async createSelector(entries: IAutosaveEntry[]): Promise<any> {
		var $modal = $('#modal-autosaveselector');
		...
		for (var entry of entries) {
			...
			$trashBtn.on('click', async (evt)=>{
				...
				if (ok && await this.deleteEntries([ entry.id ])) {
		...
		let show = UI.modal($modal, {
			onApproveCancel: () => selectedEntry.id == this.autosaveId
		});
		while (true) {
			switch (await show()) {
				case 'hide':
